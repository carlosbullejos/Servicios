...
resource "aws_internet_gateway" "vpc2_igw" {
  ...
  # Tabla de rutas para la subred pública en VPC 2
...
resource "aws_route_table_association" "vpc2_public_route_assoc" {
  subnet_id      = aws_subnet.vpc2_public_subnet.id
...
resource "aws_route_table" "vpc2_private_route_table" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.vpc2_nat_gateway.id
  }
}
resource "aws_route_table_association" "vpc2_private_route_assoc" {
  ...
  resource "aws_security_group" "bastion_sg" {
    ...
    ingress {
      ...
      to_port     = 22
      protocol    = "tcp"
      cidr_blocks = ["0.0.0.0/0"]  # IP específica para acceder al bastión
    }
